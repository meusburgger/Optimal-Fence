...
int main(int argc, char *argv[])
{
	...
	double start_time, end_time; /* for time measurement */	 
	unsigned long len;
	...
	{				
		...
		printf(":%.6lf:%.6lf\n", end_time-start_time,tpivot3-tpivot1);
	}
...
bool CalcularCercaOptima(PtrListaArboles Optimo,int rank, int size)
{
	...
	if(rank==0)
	{
		...
		for (i = 1; i < size; i++) {
			portion = (MaxCombinaciones / (size - 1)); // calculate portion without master
			low_bound = (i - 1) * portion +1;
			if (((i + 1) == size) && ((MaxCombinaciones % (size - 1)) != 0)) {//if rows of [A] cannot be equally divided among slaves
                upper_bound = MaxCombinaciones; //last slave gets all the remaining rows
            } else {
	...
	if (rank > 0) {				
		...
		//{
			for (Combinacion=low_bound; Combinacion<upper_bound; Combinacion++)
			{
	...
	int *sub_Coste = NULL;	
	int *sub_Combinacion = NULL;
	if (rank == 0) {
		sub_Coste = (int *)malloc(sizeof(int) * size);	
		sub_Combinacion= (int *)malloc(sizeof(int) * size);		
		assert(sub_Coste != NULL);		
		assert(sub_Combinacion != NULL);		
	}	
	
	// Get the total best combination cost 
	MPI_Gather(&CosteMejorCombinacion, 1, MPI_INT, sub_Coste, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);	
	//Get the Bets Combination cicle 
	MPI_Gather(&MejorCombinacion, 1, MPI_INT, sub_Combinacion, 1, MPI_FLOAT, 0, MPI_COMM_WORLD);	

	 // Now that we have all of the partial results on the root, compute the bets cost of all 	
	if(rank == 0)
	{
		CosteMejorCombinacion = Optimo->Coste;
		Combinacion = 0;
		for (i = 1; i < size - 1; i++) 
		{
			if ( sub_Coste[i] < CosteMejorCombinacion )
			{
				CosteMejorCombinacion = sub_Coste[i];
				MejorCombinacion = sub_Combinacion[i];      			
			}
		}		
		...
		printf("%ld:%ld", ArbolesEntrada.NumArboles, CosteMejorCombinacion);		
	}
